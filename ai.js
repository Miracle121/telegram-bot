...
const WRITER_INSTRUCTIONS = [
  ...
  "- The post is published as the caption under the cover image, and Telegram cuts",
  "  captions at 1024 characters. Keep the whole post under 900 characters, counting",
  "  spaces and emoji (HTML tags do not count). If the character file asks for a longer",
  "  post, this limit still wins: cut sentences, not facts.",
...
export async function ask({ history, lang, userName, mode, roleText, koverDone = false }) {
  ...
  // Qidiruv asbobi server tomonda kod bajarish muhitini ko'taradi. Sikl davom etganda
  // o'sha muhitning id'si qaytarilishi shart, aks holda API 400 beradi:
  // "container_id is required when there are pending tool uses generated by code
  // execution with tools". Birinchi so'rovda u bo'lmaydi — javob bilan birga keladi.
  let containerId = "";
  ...
  for (let turn = 1; ; turn += 1) {
    ...
    // Muhit id'si faqat server vositasi ishlatilganda keladi; kelgach saqlab qolamiz.
