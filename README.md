...
### Qidiruv siklidagi `container`

`web_search_20260209` server tomonda kod bajarish muhitini ko'taradi. Model `pause_turn`
bilan to'xtasa, keyingi so'rovga o'sha muhitning id'si qaytarilishi shart:

```js
...(containerId ? { container: containerId } : {}),
```

Busiz API 400 beradi — *"container_id is required when there are pending tool uses
